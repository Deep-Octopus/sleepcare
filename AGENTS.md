...
- `server/`: Go + Gin 后端
- `web/`: Vue 3 + Vite 前端
- `compose.yaml`、`server/Dockerfile`、`web/Dockerfile`: 本地容器化开发资产
- `docs/`: 项目文档与设计记录
- `aiDoc/`: AI 协作文档层
...
- 一个阶段尚未全部实现完成时，单个功能任务只做与当前改动直接相关的最小关键验证，例如定向单元/集成测试、必要的编译或静态检查，以及一条关键流程冒烟；默认不运行全仓测试、`make verify`、完整 Compose 重建/重启与数据幂等核对，也不重复回归已由前序任务覆盖的功能
- 阶段内单个功能默认不做页面点触测试或多角色浏览器验收；只有整个阶段实现完成进入集中验收时，或用户明确临时要求时才执行
- 整个阶段实现完成后，再集中执行全面自动化回归、Swagger/契约、前端构建、Compose/迁移与必要的浏览器验收，并复用阶段内已有证据，避免逐任务重复消耗
- 若当前改动的风险无法由最小关键验证覆盖，先说明需要扩大的验证范围和原因，获得用户确认后再执行，不自行升级为全面验收
...
- 页面点触默认遵循上面的阶段收口规则：阶段内单个功能不执行，阶段完成后集中执行；用户明确要求时除外
