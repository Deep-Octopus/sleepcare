...
## P2-06 工作人员 AI 影子能力关闭态契约

- 阶段二启用选择尚未确认，知识来源、人工审核、完整谱系、禁用场景、数据处理、评测和模型供应商输入也未完成审批，因此 P2-06 以安全关闭收口，不运行影子验证。
- `care.ai-shadow.mode` 当前只接受 `DISABLED`；任何活动模式都会阻止启动，不能通过运行配置绕过审批。
- `GET /care/ai-shadow-readiness` 返回 `selectionStatus=NOT_SELECTED`、`usageScope=NOT_ENABLED`、全部关闭的能力开关、全部未就绪的前置条件和稳定阻塞码。
- 接口只允许管家、医护和上级读取；内容管理员、普通系统管理员和缺少业务身份的请求失败关闭。它不接受用户或咨询标识，也不读取服务过程数据。
- 本任务不新增数据库实体、菜单、按钮或前端页面，不提供 propose、retrieve、review、adopt、reject、send 或任何面向康养用户的接口。
- 后续收到明确启用选择后，必须重新冻结知识版本、建议谱系、人工审核、数据处理和评测契约；本次完成状态不能解释为工作人员影子能力已经启用。
