...
.PHONY: init tools up down restart ps logs credentials swagger swagger-check backend-check backend-test frontend-check frontend-lint frontend-build phase2-rehearsal-check phase2-acceptance-check verify reset
...
phase2-acceptance-check:
	@mkdir -p "$(GO_CACHE)"
	@cd server && GOCACHE="$(GO_CACHE)" go test -mod=readonly ./config -run '^TestPhaseTwoAcceptanceContract$$' -count=1
