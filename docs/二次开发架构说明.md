...
P1-01 进一步冻结：跨模块协调属于应用用例层，不是新的领域模块，也不拥有业务表。`SubmitTaskAnswer` 是阶段一首个深接口；它从认证上下文推导访问范围，在同一事务内协调 `carePath`、`questionnaire`、`caseWork` 与 outbox，并以幂等结果作为调用方和测试的共同 seam。具体状态、错误和返回字段见 [阶段一 P1-01 领域契约与验收基线](./阶段一P1-01领域契约与验收基线.md)。
...
| ADR | 决策 | 状态 |
| --- | --- | --- |
| ADR-001 | 首期采用模块化单体，不采用微服务 | 已批准（2026-08-18） |
| ADR-002 | 普通业务使用 GVA package，不使用可安装 plugin | 已批准（2026-08-18） |
| ADR-003 | 后台员工复用 GVA 身份，康养用户使用独立业务身份/受限会话 | 已批准（2026-08-18） |
| ADR-004 | 通知、设备、AI、呼叫平台使用 port + adapter | 已批准（2026-08-18；对应能力仍受启用门禁约束） |
| ADR-005 | 核心业务使用本地事务 + outbox，不引入消息中间件 | 已批准（2026-08-18） |
| ADR-006 | 阶段一核心闭环真实落库，非核心只读投影可 Mock | 已批准（2026-08-18；Mock 必须醒目标注） |
| ADR-007 | 敏感业务缺少身份、组织或责任上下文时 fail-closed | 已批准（2026-08-18） |
